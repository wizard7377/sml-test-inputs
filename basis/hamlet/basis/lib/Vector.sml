...
struct
  ...
  val maxLen     = Vector.maxLen  () : int
  fun length x   = (Vector.length  : 'a vector -> int) x
  fun sub x      = (Vector.sub  : 'a vector * int -> 'a) x
  fun fromList x = (Vector.fromList  : 'a list-> 'a vector) x
